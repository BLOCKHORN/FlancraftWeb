...
const obtenerSaleActiva = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, active: false, sale: null, error: 'No se pudo obtener la sale activa' });
  }
};
  ...
  try {
    ...
    if (!checkoutUrl) {
      return res.status(502).json({
        ...
        detail:
          finalBasket?.data?.links || created?.data?.links || null,
      });
    }
...
const obtenerSidebarRaw = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      ...payload,
      cacheado: new Date(
        headlessCache.sidebarRaw.cacheAt * 1000
      ).toISOString(),
    });
  } catch (e) {
    return res.status(500).json({
...
const obtenerTopDonator = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      ...payload,
      cacheado: new Date(
        headlessCache.topDonator.cacheAt * 1000
      ).toISOString(),
    });
  } catch (e) {
    return res.status(500).json({
...
const obtenerPagosRecientes = async (req, res) => {
  try {
    ...
    const payments = Array.isArray(mod?.data?.payments)
      ? mod.data.payments
      : [];

    headlessCache.recentPayments = {
      data: payments,
      cacheAt: nowSec(),
    };
    ...
    return res.json({
      ...
      payments,
      cacheado: new Date(
        headlessCache.recentPayments.cacheAt * 1000
      ).toISOString(),
    });
  } catch (e) {
    return res.status(500).json({
...
const obtenerBasketHeadless = async (req, res) => {
  ...
  try {
    ...
    if (!token)
      return res
        .status(500)
        .json({ ok: false, error: 'Falta TEBEX_WEBSTORE_TOKEN.' });

    const ident = String(req.params.ident || '').trim();
    if (!ident)
      return res
        .status(400)
        .json({ ok: false, error: 'Falta basket ident.' });
  ...
  try {
    ...
    if (!['creator', 'coupon', 'giftcard', 'coupon_giftcard'].includes(tipo)) {
      return res.status(400).json({
        ok: false,
        error:
          'Tipo invalido. Usa "creator", "coupon", "giftcard" o "coupon_giftcard".',
      });
    }
    if (!codigo)
...
const agregarPaqueteBasket = async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(package_id) || package_id <= 0) {
      return res
        .status(400)
        .json({ ok: false, error: 'package_id invalido.' });
    }
    if (!Number.isFinite(quantity) || quantity <= 0) {
      return res
        .status(400)
        .json({ ok: false, error: 'quantity invalida.' });
    }

    const getUrl = `https://headless.tebex.io/api/accounts/${encodeURIComponent(
      token
    )}/baskets/${encodeURIComponent(ident)}`;
    const basketRes = await headlessFetchJson({
      rid,
      url: getUrl,
      method: 'GET',
    });
...
const obtenerRecomendaciones = async (req, res) => {
  ...
  const count = Math.max(
    1,
    Math.min(6, Number(req.query.count || 3))
  );
  ...
  try {
    ...
    const list = (cache[server].paquetes || [])
      .filter(
        (p) =>
          p && !exclude.includes(Number(p.id ?? p.package_id))
      )
      .map((p) => {
...
const webhookHandler = async (req, res) => {
  try {
    if (!WEBHOOK_SECRET) {
      return res
        .status(500)
        .json({ ok: false, error: 'Falta TEBEX_WEBHOOK_SECRET' });
    }
    ...
    if (!raw || !Buffer.isBuffer(raw))
      return res
        .status(400)
        .json({ ok: false, error: 'Missing raw body' });
